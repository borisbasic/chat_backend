...
class Rooms(Base):
    ...
    videos = relationship("Videos", back_populates="room")
...
class Videos(Base):
    ...
    sender_video = Column(Integer, ForeignKey("users.id"))
    receiver_video = Column(Integer, ForeignKey("users.id"))
    room_id = Column(Integer, ForeignKey("rooms.id"))
    room = relationship("Rooms", back_populates="videos")
