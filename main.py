...
from schemas import MessageCreate, UserBase, RoomNumber, Login,  ImageDisplay, ChatImage \
    , DocumentDisplay, ChatDocuments
from database.database import get_db
from database.models import User, Message, Rooms,Images, Documents
...
origins = [
    'http://localhost:3000' # we use this for our web application
]
...
async def websocket_endpoint(websocket: WebSocket, room_id: int,
    ...
    try:
        while True:
            ...
            new_room_message = Message(sender_id=data['sender_id'], receiver_id=data['receiver_id'],
...
def create_message(message: MessageCreate, db: Session = Depends(get_db)):
    db_message = Message(sender_id = message.sender_id,
...
async def get_messages(sender_id: int, receiver_id: int, db: Session = Depends(get_db)):
    ...
    messages = db.query(Message).filter(or_(and_(Message.sender_id == sender_id, Message.receiver_id == receiver_id)
                                        , and_(Message.sender_id == receiver_id, Message.receiver_id == sender_id))).all()
...
def get_room_number(sender_id: int, receiver_id: int, db: Session = Depends(get_db)):
    room = db.query(Rooms).filter(or_(and_(Rooms.sender_id == sender_id, Rooms.receiver_id == receiver_id)
                                        , and_(Rooms.sender_id == receiver_id, Rooms.receiver_id == sender_id))).first()
    if room:
        return room
    else:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def post_chat_image(request: ChatImage, db: Session = Depends(get_db)):
    new_image = Images(image_name=request.image_name,
...
def post_chat_image(request: ChatDocuments, db: Session = Depends(get_db)):
    new_document = Documents(document_name=request.document_name,
...
def chceck_exist_image(document_name: str):
